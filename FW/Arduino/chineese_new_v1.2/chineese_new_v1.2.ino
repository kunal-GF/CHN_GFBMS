...
/**Communication command for dashboard*/
#define COMM_CMD_VON         10  
/**Communication command for dashboard*/
#define COMM_CMD_VOFF        11   
/**Communication command for dashboard*/
#define COMM_CMD_LON         12  
/**Communication command for dashboard*/
#define COMM_CMD_LOFF        13              
...
#define SZ_CMD_VON            3      
/**Size for data log command*/
#define SZ_CMD_VOFF           4   
/**Size for data log command*/
#define SZ_CMD_LON            3      
/**Size for data log command*/
#define SZ_CMD_LOFF           4              
...
const char cmd_von[SZ_CMD_VON]        =  "von";
const char cmd_voff[SZ_CMD_VOFF]      =  "voff";
const char cmd_lon[SZ_CMD_LON]        =  "lon";
const char cmd_loff[SZ_CMD_LOFF]      =  "loff";
...
unsigned long keyrhgerval = 0;
unsigned long keyrhgpreviousmilis = 0;

unsigned long ocscerval = 0;
unsigned long ocscpreviousmilis = 0;

unsigned long ocderval = 5000;
unsigned long ocdpreviousmilis = 0;

uint16_t celldiff = 0x0000;

uint8_t vbstat = 0x00,lstat = 0x00;
...
void afe_initz(void)
{
   ...
   */
   ...
   i2c_WriteWithCRC(I2C_7BITADDR, PROTECT1, 0x12);
   i2c_WriteWithCRC(I2C_7BITADDR, PROTECT2, 0x08);
   i2c_WriteWithCRC(I2C_7BITADDR, PROTECT3, afe_isr_reg[132]);  
   i2c_WriteWithCRC(I2C_7BITADDR, CC_CFG, 0x19);
   i2c_WriteWithCRC(I2C_7BITADDR, OV_TRIP, afe_isr_reg[125]);
   i2c_WriteWithCRC(I2C_7BITADDR, UV_TRIP, afe_isr_reg[126]);
...
void ldsett(void)
{
  ...
  /**Load short circuit times high bits*/
    afe_isr_reg[67] = eeprom_read_byte(0x28);
  /**Load short circuit times lower bits*/
    afe_isr_reg[68] = eeprom_read_byte(0x29);
  ...
  /**Key-in time out*/
  afe_isr_reg[120] = eeprom_read_byte(0x30);
  ...
  afe_isr_reg[125] = eeprom_read_byte(0x09);

  afe_isr_reg[126] = eeprom_read_byte(0x0E);
     ...
     afe_isr_reg[87] = eeprom_read_byte(0x44);
      ...
      afe_isr_reg[88] = eeprom_read_byte(0x45);
       ...
       afe_isr_reg[85] = eeprom_read_byte(0x46);
        ...
        afe_isr_reg[17] = eeprom_read_byte(0x48);
        afe_isr_reg[18] = eeprom_read_byte(0x49);

        afe_isr_reg[19] = eeprom_read_byte(0x4A);
        afe_isr_reg[20] = eeprom_read_byte(0x4B);

        afe_isr_reg[21] = eeprom_read_byte(0x4C);
        afe_isr_reg[22] = eeprom_read_byte(0x4D);

        afe_isr_reg[23] = eeprom_read_byte(0x4E);
        afe_isr_reg[24] = eeprom_read_byte(0x4F);

        afe_isr_reg[25] = eeprom_read_byte(0x50);
        
        afe_isr_reg[26] = eeprom_read_byte(0x51);

        afe_isr_reg[27] = eeprom_read_byte(0x52);
        afe_isr_reg[28] = eeprom_read_byte(0x53);

        afe_isr_reg[29] = eeprom_read_byte(0x54);
        afe_isr_reg[30] = eeprom_read_byte(0x55);

        afe_isr_reg[139] = eeprom_read_byte(0x59);
        afe_isr_reg[140] = eeprom_read_byte(0x5A);
        
       
         keyrhgerval = afe_isr_reg[120]*0xEA60;
         ocscerval = afe_isr_reg[107]*0xEA60;
...
void db(void)
{
  ...
  if (!i2c_start_wait((I2C_7BITADDR<<1)|I2C_WRITE))
    ...
    i2c_write(0x09);
    i2c_rep_start((I2C_7BITADDR<<1)|I2C_READ);
    afe_isr_reg[125] = i2c_read(true);

    i2c_write(0x09);
    i2c_rep_start((I2C_7BITADDR<<1)|I2C_READ);
    afe_isr_reg[126] = i2c_read(true);
    i2c_stop();
}
...
void bsbi(void)
{
  ...
  celldiff = cmax-cmin;
  if(celldiff > merge_16(afe_isr_reg[139], afe_isr_reg[140]))
  {
    afe_isr_reg[139] = (celldiff&0xFF00)>>7;
    afe_isr_reg[140] =  celldiff&0x00FF;
    eeprom_write_byte(0x59, afe_isr_reg[139]);
    eeprom_write_byte(0x5A, afe_isr_reg[140]);
  }
}
      ...
      if(comm_ptr_next[1] == 'w')
      {
        ...
        /*
          ...
          */
          ...
          i2c_WriteWithCRC(I2C_7BITADDR, OV_TRIP, afe_isr_reg[125]);
          eeprom_write_byte(0x09, afe_isr_reg[125]);
          TU_putHex(0xFF);
          break;
          ...
          i2c_WriteWithCRC(I2C_7BITADDR, UV_TRIP, afe_isr_reg[126]);
          eeprom_write_byte(0x0E, afe_isr_reg[126]);
          TU_putHex(0xFF);
          break;
          ...
          eeprom_write_byte(0x30, afe_isr_reg[120]);
          TU_putHex(0xFF);
          break;
          ...
          h=0;
          h = (str2hex(comm_ptr_next[6])<<4) | (str2hex(comm_ptr_next[7]) & 0xff);
          i2c_WriteWithCRC(I2C_7BITADDR, SYS_CTRL2, h);
          break;

          case 0x8A :
          h=0;
          h = (str2hex(comm_ptr_next[6])<<4) | (str2hex(comm_ptr_next[7]) & 0xff);
          i2c_WriteWithCRC(I2C_7BITADDR, PROTECT1, h);
          break;
       }
      }
      ...
      if(comm_ptr_next[1] == 'r')
      {
        ...
        /*
          ...
          */
          ...
          i2c_write(OV_TRIP);
          i2c_rep_start((I2C_7BITADDR<<1)|I2C_READ);
          TU_putHex(i2c_read(true));
          i2c_stop();
          TU_putHex(afe_isr_reg[125]);
          TU_putHex(eeprom_read_byte(0x09));
          break;
          ...
          TU_putHex(i2c_read(true));
          i2c_stop(); 
          TU_putHex(afe_isr_reg[126]);
          TU_putHex(eeprom_read_byte(0x0E));
          break;

          case 0x78 : 
          TU_putHex(afe_isr_reg[120]);
          break;

          case 0x8B : 
          TU_putHex(afe_isr_reg[139]);
          TU_putHex(afe_isr_reg[140]);
          break;
...
uint8_t process(void)
{
  ...
  do{
    ...
    if(strncmp(cmd_von, comm_buffer, SZ_CMD_VON) == 0)
    {
    ...
    if(strncmp(cmd_voff, comm_buffer, SZ_CMD_VOFF) == 0)
    {
    ...
    if(strncmp(cmd_loff, comm_buffer, SZ_CMD_LOFF) == 0)
    {
     ...
     if(strncmp(cmd_lon, comm_buffer, SZ_CMD_LON) == 0)
    {
...
void trip(void)
{
  ...
  if((afe_isr_reg[127] & 0x7F) > 0)
    {
      /**Short circuit in discharging*/
      if((afe_isr_reg[127]&0x02) == 0x02)
      {
        afe_isr_reg[1] |= (1<<0);
        verb_print(0x0019);

        /**store pack max current at which short circuit fault occured*/
        afe_isr_reg[31] = afe_isr_reg[4];
        afe_isr_reg[32] = afe_isr_reg[5];

        eeprom_update_byte(0x01, afe_isr_reg[1]);
        eeprom_update_byte(0x56, afe_isr_reg[31]);
        eeprom_update_byte(0x57, afe_isr_reg[32]);

        if(afe_isr_reg[68] == 0xFF)
        {
          afe_isr_reg[67] += 1;
          afe_isr_reg[68] = 0x00;  
        }
        ...
        eeprom_update_byte(0x28, afe_isr_reg[67]);
        eeprom_update_byte(0x29, afe_isr_reg[68]);
      }

      /**Over current in discharging*/
      if((afe_isr_reg[127]&0x02) == 0x01)
      {
        afe_isr_reg[1] |= (1<<1);
        verb_print(0x001B);
      }
...
if( (!(afe_isr_reg[0] & 0x80)) && (rhgtog == 0x00) )
{
if((afe_isr_reg[127]&0x04) || (cmax > merge_16(afe_isr_reg[87],afe_isr_reg[88])))
  {
    i2c_WriteWithCRC(I2C_7BITADDR, SYS_CTRL2, 0x42); 
    afe_isr_reg[0] |= (1<<7); 
    
    /**Cell max voltage at which fault occured*/
    afe_isr_reg[17] = (cmax&0xFF00)>>7;
    afe_isr_reg[18] = cmax&0x00FF;
    TU_putHexW(cmax);
    eeprom_update_byte(0x00, afe_isr_reg[0]);
    eeprom_update_byte(0x48, afe_isr_reg[17]);
    eeprom_update_byte(0x49, afe_isr_reg[18]);
  ...
  if(((afe_isr_reg[127]&0x08) || (cmin < merge_16(afe_isr_reg[85],afe_isr_reg[86]))) && !(afe_isr_reg[0] & 0x40))
  {
    ...
    afe_isr_reg[0] |= (1<<6); 
     ...
     /**Cell min voltage at which fault occured*/
    afe_isr_reg[19] = (cmax&0xFF00)>>7;
    afe_isr_reg[20] = cmax&0x00FF;
    
    eeprom_update_byte(0x00, afe_isr_reg[0]);
    eeprom_update_byte(0x4A, afe_isr_reg[19]);
    eeprom_update_byte(0x4B, afe_isr_reg[20]);
   ...
   /**pack overvoltage fault*/
    if((merge_16(afe_isr_reg[2],afe_isr_reg[3]) > merge_16(afe_isr_reg[89],afe_isr_reg[90])) && (!(afe_isr_reg[0] & 0x20)))
    {
     ...
     /**Pack overvoltage at which fault occured*/
    afe_isr_reg[21] = afe_isr_reg[2];
    afe_isr_reg[22] = afe_isr_reg[3];
    
    eeprom_update_byte(0x00, afe_isr_reg[0]);
    eeprom_update_byte(0x4C, afe_isr_reg[21]);
    eeprom_update_byte(0x4D, afe_isr_reg[22]);
  ...
  /**pack undervoltage fault*/
    if((merge_16(afe_isr_reg[2],afe_isr_reg[3]) < merge_16(afe_isr_reg[91],afe_isr_reg[92])) && (!(afe_isr_reg[0] & 0x10)))
    {
      ...
      afe_isr_reg[0] |= (1<<4); 
       ...
       /**Pack overvoltage at which fault occured*/
      afe_isr_reg[23] = afe_isr_reg[2];
      afe_isr_reg[24] = afe_isr_reg[3];
      
      eeprom_update_byte(0x00, afe_isr_reg[0]);
      eeprom_update_byte(0x4E, afe_isr_reg[23]);
      eeprom_update_byte(0x4F, afe_isr_reg[24]);
 ...
 if(!(merge_16(afe_isr_reg[4],afe_isr_reg[5])& 0x8000) && !(afe_isr_reg[4] & 0x02))
 {
    if(merge_16(afe_isr_reg[4],afe_isr_reg[5]) > merge_16(afe_isr_reg[93],afe_isr_reg[94]))
    {
      if(millis() - ocdpreviousmilis >= ocderval)
      {
        ocdpreviousmilis = millis();
      i2c_WriteWithCRC(I2C_7BITADDR, SYS_CTRL2, 0x42);
      TU_putHexW(0x0002);
      keytog = 0x00;
      afe_isr_reg[0] |= (1<<1); 

      afe_isr_reg[27] = afe_isr_reg[4];
      afe_isr_reg[28] = afe_isr_reg[5];
      
      eeprom_update_byte(0x00, afe_isr_reg[0]);
      eeprom_update_byte(0x52, afe_isr_reg[27]);
      eeprom_update_byte(0x53, afe_isr_reg[28]);
  ...
  if((afe_isr_reg[4]&0x80) > 0)
  {
    ...
    if(merge_16(afe_isr_reg[4],afe_isr_reg[5]) < merge_16(afe_isr_reg[95],afe_isr_reg[96]))
         ...
         if(millis() - ocdpreviousmilis >= ocderval)
      {
        ...
        afe_isr_reg[29] = afe_isr_reg[4];
        afe_isr_reg[30] = afe_isr_reg[5];
        
        eeprom_update_byte(0x00, afe_isr_reg[0]);
        eeprom_update_byte(0x54, afe_isr_reg[29]);
        eeprom_update_byte(0x55, afe_isr_reg[30]);
      ...
      //TU_putHex(rhgtog);
      if(  (rhgtog == 0x40) && (merge_16(afe_isr_reg[4],afe_isr_reg[5]) < 0xFFED) )
      {
        i2c_WriteWithCRC(I2C_7BITADDR, SYS_CTRL2, 0x41);
        verb_print(0x0006);
        rhgtog = 0x00;
  ...
  if((afe_isr_reg[137] > afe_isr_reg[97]) && (!(afe_isr_reg[0] & 0x08)))
  {
    ...
    afe_isr_reg[0] |= (1<<3);
    afe_isr_reg[25] = afe_isr_reg[137];

    eeprom_update_byte(0x00, afe_isr_reg[0]);
    eeprom_update_byte(0x54, afe_isr_reg[25]);

    if(afe_isr_reg[82] == 0xFF)
      {
        afe_isr_reg[82] += 1;
        afe_isr_reg[81] = 0x00;
      }
...
void rel(void)
{
  ...
  /**Short circuit fault released*/
  if((afe_isr_reg[1] & 0x01))
  {
    afe_isr_reg[1] &= ~(1<<0);
    eeprom_update_byte(0x01, afe_isr_reg[1]);
    verb_print(0x001A);
  ...
  /**overcurrent in  fault released*/
  if((afe_isr_reg[1] & 0x02))
  {
    afe_isr_reg[1] &= ~(1<<1);
    eeprom_update_byte(0x01, afe_isr_reg[1]);
    verb_print(0x001C);
...
void chk_key_act(void)
{
  ...
  { 
    if(merge_16(afe_isr_reg[4],afe_isr_reg[5]) < 0x0018)
    {
      i2c_WriteWithCRC(I2C_7BITADDR, SYS_CTRL2, 0x40);
      keytog = 0x00;
      verb_print(0x0003);
    ...
    if(merge_16(afe_isr_reg[4],afe_isr_reg[5]) >= 0xFFE8)
    {
      i2c_WriteWithCRC(I2C_7BITADDR, SYS_CTRL2, 0x40);
      keytog = 0x00;
      verb_print(0x0004);
