...
void ldsett(void)
{
  ...
  //afe_isr_reg[] = eeprom_read_byte(0x31);
  //afe_isr_reg[] = eeprom_read_byte(0x32);
...
void configw(void)
{
  ...
  if(comm_cmd_sz != 0 && (comm_sz - comm_cmd_sz) == 2)
  {
    // Process the next part
    num = str2hex(comm_ptr_next[1]);
    num += str2hex(comm_ptr_next[0])<<4;
    // Check the Number
    if(num == 10)
    {
      // This code is authentic
      TU_putln(" BL: !! Received Fault Clear Command !!");      
      // Loop through all Faults and clear them
      for(i=0;i<SYS_MAX_FAULTS;i++)
        hre_clear16(eep.xa_faults[i].u8a_index_arr, SYS_EEP_RELIBILITY_DEPTH);
      // Clear the Max Fault Status
      u16_Sys_Status &= ~SYS_CONFIG_MAX_FAULTS;
      // TODO: Update System Status to EEPROM
      TU_putln(" BL: !! Cleaning All Faults !! ");
      return;
    }// End of Code Check
  } // End of Further Processing to interpret 2 Bytes
  TU_putln(" BL: ERROR Received Wrong Fault Clear Command");          
}
